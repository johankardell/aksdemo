...
export K8s_version='1.29' # Use older version and update via Fleet manager - or just use newer version
...
(
    ...
    az aks get-credentials -n $AKS_CLUSTER_NAME_1 -g $AKS_RESOURCE_GROUP --subscription $SUBSCRIPTION_ID1
) &
...
(
    ...
    az aks get-credentials -n $AKS_CLUSTER_NAME_2 -g $AKS_RESOURCE_GROUP --subscription $SUBSCRIPTION_ID2
) &
...
(
    ...
    az aks nodepool add --resource-group $AKS_RESOURCE_GROUP --cluster-name $AKS_CLUSTER_NAME_3 --ssh-access disabled --name storage --node-count 3 --node-vm-size Standard_L8as_v3 --os-type Linux --os-sku AzureLinux --subscription $SUBSCRIPTION_ID3
    ...
    az aks get-credentials -n $AKS_CLUSTER_NAME_3 -g $AKS_RESOURCE_GROUP --subscription $SUBSCRIPTION_ID3
) &
...
(
    ...
    az aks get-credentials -n $AKS_CLUSTER_NAME_4 -g $AKS_RESOURCE_GROUP --subscription $SUBSCRIPTION_ID4
) &
...
(
    ...
    az aks get-credentials -n $AKS_CLUSTER_NAME_5 -g $AKS_RESOURCE_GROUP --subscription $SUBSCRIPTION_ID5
) &

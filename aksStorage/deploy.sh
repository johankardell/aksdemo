...
(
    ...
    az aks nodepool add --resource-group $AKS_RESOURCE_GROUP --cluster-name $AKS_CLUSTER_NAME_1 --ssh-access disabled --name storage --node-count 3 --node-vm-size Standard_D8lds_v6 --node-osdisk-type Ephemeral --os-type Linux --os-sku AzureLinux --subscription $SUBSCRIPTION_ID1
...
fio --directory=/volume --size=4000MB --filename_format='testfile.$jobnum' --wait_for_previous --thread --group_reporting --direct=1 --randrepeat=0 --norandommap=1 --ioengine=io_uring --numjobs=8 --name=precondition --bs=1M --iodepth=64 --rw=write --name=randwritebench --rw=randwrite --bs=4k --iodepth=16 --time_based --runtime=60 --name=randreadbench --rw=randread --bs=4k --iodepth=16 --time_based --runtime=60 --name=seqwritebench --rw=write --bs=128k --iodepth=16 --time_based --runtime=60 --name=seqreadbench --rw=read --bs=128k --iodepth=16 --time_based --runtime=60
